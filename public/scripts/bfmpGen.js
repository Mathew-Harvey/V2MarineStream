...
document.addEventListener('DOMContentLoaded', function() {
    // No modal needed - show generator content directly
...
function initBiofoulingPlanGenerator() {
    const generatorContainer = document.querySelector('.generator-content') || document.getElementById('plan-generator-modal');
    ...
    function updateProgressIndicator() {
        const tabs = generatorContainer.querySelectorAll('.tab-btn');
        const progressSteps = generatorContainer.querySelectorAll('.progress-step');

        // Find the active tab
        const activeTab = generatorContainer.querySelector('.tab-btn.active');
        if (!activeTab) return;
    ...
    const tabButtons = generatorContainer.querySelectorAll('.tab-btn');
    const tabPanes = generatorContainer.querySelectorAll('.tab-pane');
    ...
    const nextButtons = generatorContainer.querySelectorAll('.next-tab');
    const prevButtons = generatorContainer.querySelectorAll('.prev-tab');
    ...
    nextButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (nextTabId) {
                ...
                const nextTabButton = generatorContainer.querySelector(`.tab-btn[data-tab="${nextTabId}"]`);
                if (nextTabButton) {
                    nextTabButton.click();
                }
            }
    ...
    prevButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            if (prevTabId) {
                // Find and click the corresponding tab button
                const prevTabButton = generatorContainer.querySelector(`.tab-btn[data-tab="${prevTabId}"]`);
                if (prevTabButton) {
                    prevTabButton.click();
                }
            }
    ...
    function setupFileUploadPreview(inputId, previewId) {
        ...
        const previewContainer = document.getElementById(previewId) || (fileInput && fileInput.getAttribute('data-preview') ? document.getElementById(fileInput.getAttribute('data-preview')) : null);
    ...
    // Feature flag: disable built-in PDF download in favor of preview + print-to-PDF (more reliable across browsers)
    ...
    function getFileDataUrl(id) {
        ...
        // Prefer preview container referenced via data-preview
        const previewContainerId = fileInput.getAttribute('data-preview');
        if (previewContainerId) {
            const previewContainer = document.getElementById(previewContainerId);
            const firstPreview = previewContainer && previewContainer.querySelector('.preview-img');
            if (firstPreview) return firstPreview.src;
        }
        // If no preview present, derive a URL directly from file input for images
        ...
        // Fallback: first image anywhere under the same form-group
    ...
    function getFileDataUrls(id) {
        ...
        // Use explicit data-preview container if present, else derive
        const previewContainerId = fileInput.getAttribute('data-preview') || id.replace('Files', '-preview');
        const previewContainer = document.getElementById(previewContainerId);
    ...
    if (generateButton && !disablePdfDownload) {
            ...
            try {
                ...
                if (planData) {
                    ...
                    if (typeof html2pdf !== 'undefined') {
                        // Create an offscreen container for reliable rendering (keep in viewport but invisible)
                        const element = document.createElement('div');
                        element.style.cssText = 'position: absolute; left: 0; top: 0; width: 794px; background: #fff; visibility: hidden; pointer-events: none;';
                        element.innerHTML = planHtml;
                        // Override preview-only styles for PDF render
                        const style = document.createElement('style');
                        style.textContent = '.report-preview{max-height:none !important; overflow:visible !important; box-shadow:none !important; border:0 !important;} @page{size:A4; margin:10mm;}';
                        ...
                        // Wait for all images inside to load to avoid blank pages
                        ...
                        html2pdf().set(options).from(element).save().then(() => {
                            // Revoke temp blob URLs if used
                            Array.from(element.querySelectorAll('img'))
                                .map(img => img.getAttribute('src'))
                                .filter(src => src && src.startsWith('blob:'))
                                .forEach(src => { try { URL.revokeObjectURL(src); } catch (_) {} });
    ...
    if (closePreviewButton && planPreviewModal) {
        closePreviewButton.addEventListener('click', function () {
            planPreviewModal.classList.remove('active');
        });
    }
    ...
    // If PDF download is disabled, hide download buttons and guide the user to Print -> Save as PDF
    if (disablePdfDownload) {
        if (generateButton) {
            generateButton.style.display = 'none';
        }
        if (downloadPlanButton) {
            downloadPlanButton.style.display = 'none';
        }
        if (printPlanButton) {
            printPlanButton.textContent = 'Print / Save as PDF';
        }
        const generateTab = document.getElementById('generate');
        if (generateTab) {
            const info = document.createElement('div');
            info.className = 'info-note';
            info.innerHTML = '<p><i class="fas fa-info-circle"></i> To download a PDF, click Preview Plan, then click "Print / Save as PDF" and choose "Save as PDF" in your print dialog.</p>';
            const actions = generateTab.querySelector('.form-actions.button-group');
            if (actions) {
                actions.parentNode.insertBefore(info, actions);
            }
        }
    }
    if (downloadPlanButton && planPreviewContainer && !disablePdfDownload) {
        downloadPlanButton.addEventListener('click', async function () {
            try {
                const planData = collectPlanData();
                if (planData) {
                    const planHtml = generatePlanHtml(planData);
                    ...
                    if (typeof html2pdf !== 'undefined') {
                        // Create an offscreen container to avoid capturing hidden/transformed content
                        const element = document.createElement('div');
                        element.style.cssText = 'position: fixed; left: -10000px; top: 0; width: 800px; background: #fff; z-index: -1;';
                        element.innerHTML = planHtml;
                        document.body.appendChild(element);
                        ...
                        html2pdf().set(options).from(element).save().then(() => {
                            // Revoke any temporary blob: URLs used by images
                            Array.from(element.querySelectorAll('img'))
                                .map(img => img.getAttribute('src'))
                                .filter(src => src && src.startsWith('blob:'))
                                .forEach(src => { try { URL.revokeObjectURL(src); } catch (_) {} });
                            document.body.removeChild(element);
                            document.body.removeChild(loadingIndicator);
                            // Close preview modal properly
                            const previewModalEl = document.getElementById('plan-preview-modal');
                            if (previewModalEl) {
    ...
    // Print plan from preview into a dedicated window
    if (printPlanButton && planPreviewContainer) {
        printPlanButton.addEventListener('click', async function() {
            try {
                const planHtml = planPreviewContainer.innerHTML;
                ...
                // Build a minimal, print-friendly document
                const doc = printWindow.document;
                doc.open();
                doc.write(`<!DOCTYPE html><html><head><title>BFMP Print</title>
                    <meta charset="utf-8"/>
                    ...
                    <style>
                        ...
                        .report-preview { box-shadow: none !important; border: 0 !important; max-width: 100% !important; margin: 0 !important; padding: 0 !important; }
                ...
                await new Promise(res => {
                    ...
                    imgs.forEach(img => {
                        if (img.complete) {
                            ...
                            img.onload = img.onerror = () => { if (--remaining === 0) res(); };
                        }
                    });
                });
                ...
                console.error('Error printing plan:', err);
    ...
    modals.forEach(modal => {
        modal.addEventListener('click', function (event) {
            if (event.target === modal) {
                ...
                // Clear any inline style so CSS .active controls visibility
                modal.style.display = '';
            }
        });
    });
...
function generatePlanHtml(data) {
    ...
    // Plan format handling and dynamic table of contents
    ...
    return `
        <div class="report-preview">
            ...
            <div class="section">
                ...
                ${data.iafs.file ? `<p><img src="${data.iafs.file}" alt="IAFS Certificate" style="max-width: 100%;"></p>` : '<p class="placeholder-text">Upload a copy of the IAFS certificate (image formats recommended) to include here. If a PDF is uploaded it will be referenced but not embedded in this PDF.</p>'}
